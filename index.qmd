...
## Do we need that for the NA?
#cantril_table <-cantril %>%  
 #filter( Year == 2011 | Year == 2023) %>% 
  #pivot_wider(values_from = `Cantril ladder score`, names_from = Year) %>% 
  #select(-Code) %>% 
  #mutate(`Absolut change`= if_else(!is.na(`2011`) | !is.na(`2023`), `2023` - `2011`, NA_real_),
         #`Relative change`= if_else(!is.na(`2011`) & !is.na(`2023`), scales::percent(round((`2023` - `2011`) / `2011`, 2)), NA_character_)) 
...
cantril_table <-cantril %>%  
  ...
  pivot_wider(values_from = `Cantril ladder score`, names_from = Year) %>% 
  select(-Code) %>% 
  mutate(`Absolut change`= round(`2023` - `2011`,2),
         `Relative change`= scales::percent(round((`2023` - `2011`) / `2011`, 2))) 
...
Cantril_chart <-cantril %>% 
  filter(Country == "Armenia" | Country == "Brazil" | Country == "Canada" | Country == "Switzerland") 

Cantril_chart_plot <- Cantril_chart %>% 
  ggplot(mapping=aes(x=Year, y=`Cantril ladder score`)) +
  geom_line(mapping=aes(color=Country))+
  scale_y_continuous(limits = c(0,10))+
  labs(title= "Cantril score over time") +
  theme_minimal()
